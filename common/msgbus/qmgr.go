...
func (em *exchangeManagerImpl) createDiscoveryExchangeIfNotExists() *exchangeManagerImpl {
	err := em.channel.ExchangeDeclare(
		...
		exchangeType.string(fanoutExchange),
		false,
		true,
		false,
		false,
...
func (em *exchangeManagerImpl) createCommandExchangeIfNotExists() *exchangeManagerImpl {
	err := em.channel.ExchangeDeclare(
		...
		exchangeType.string(directExchange),
		false,
		true,
		false,
		false,
...
func (qm *queueManagerImpl) bindToQueue(exchange exchange) *queueManagerImpl {
...
func (qm *queueManagerImpl) publishMessage(exchange string , queueName string, message amqp.Publishing) {
	err := qm.channel.Publish(exchange,queueName,
		false,
		false, 	    //if true than throws error when no consumers on the q
		message)
	utils.FailOnError(err, "default publishing")
