function createChart(parentNode, data) {
  ...
  $('.' + data.name).CanvasJSChart({
    ...
    axisX: {
      labelFormatter: function(e) {https://www.hrubaskala.info/album/fotogalerie-zamek-hruba-skala/hruba-skala-jarolimek-09-jpg/
  ...
  chart = $('.' + data.name)[0];
  chart['minSafeValue'] = data.minSafeValue;
  chart['maxSafeValue'] = data.maxSafeValue;
}
...
function updateChart(msg) {
  ...
  chart.options.data[1].dataPoints[0] = {x: pts[0].x, y: [minSafeValue, maxSafeValue]};
  chart.options.data[1].dataPoints[1] = {x: pts[pts.length-1].x, y:[minSafeValue, maxSafeValue]};
  chart.render();
}