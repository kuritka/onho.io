...
func (s *Server) handleHome(sessionKey string)http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		err = s.templates["home.html"].ExecuteTemplate(w, "base", renderData)
...
func (s *Server) serveWebSockets() http.HandlerFunc {
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		socket.SetPongHandler(func(string) error {
			...
			err = socket.SetWriteDeadline(time.Now().Add(closeConnectionDuration))
			utils.FailFastOnError(err)
			err = socket.SetReadDeadline(time.Now().Add(closeConnectionDuration))
			utils.FailFastOnError(err)
			return nil
		})
